...
from HAND.options import PermutationsConfig


class PermutationsFactory:
    @staticmethod
    def get(permutations_cfg: PermutationsConfig):
        if permutations_cfg.permute_mode == "joint":
            return JointPermutations(permutations_cfg.num_workers)
        elif permutations_cfg.permute_mode == "separate":
            return SeparatePermutations()
        else:
            raise ValueError("Unsupported permutations mode")


class JointPermutations:
