...
parser.add_argument('--pruned-weights-path', default='', type=str, metavar='PATH',
                    help='path to the pruned weights indices')
best_prec1 = 0
...
def main():
    ...
    train_loader, val_loader = get_dataloaders_fn({'batch_size': args.batch_size, 'num_workers': args.workers},
                                                  {'num_workers': args.workers}, use_workers=True)

    # load pruned weights indices
    pruned_weights_indices = torch.load(args.pruned_weights_path)

    # freeze gradients for pruned weights
    for layer_params, layer_prune_indices in zip(model.module.get_learnable_weights(), pruned_weights_indices):
        layer_params = layer_params.flatten()
        for index, weight in enumerate(layer_params):
            if index in layer_prune_indices:
...
def save(model, suffix=""):
    ...
    if args.save_model:
        ...
        torch.save(model.module.state_dict(),
                   save_dir + "/" + args.exp_name + ('.pt' if not suffix else f'_{suffix}.pt'))
        with open(save_dir + '/' + args.exp_name + ('.json' if not suffix else f'_{suffix}.json'),
                  'w') as model_save_path:
