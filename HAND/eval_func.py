...
class EvalFunction:
    ...
    def eval(self, reconstructed_model: Union[ReconstructedModel, ReconstructedDataParallel],
        ...
        if self.cfg.task.task_name == "imagenet":
            return self._logged_imagenet_eval(reconstructed_model, dataloader, epoch, clearml_logger, suffix)
        else:
            return self._basic_eval(reconstructed_model, dataloader, epoch, clearml_logger, suffix)
    ...
    def _logged_imagenet_eval(self, reconstructed_model: Union[ReconstructedModel, ReconstructedDataParallel],
        ...
        if clearml_logger is not None:
            for m in metrics.keys():
                clearml_logger.report_scalar(f'{m}{f"_{suffix}" if suffix else ""}', m, metrics[m], epoch)
