...
class HANDPredictorBase(nn.Module, ABC):
    ...
    @staticmethod
    def predict_all(predictor: 'HANDPredictorBase', positional_embeddings: List[torch.Tensor],
        ...
        if predictor.cfg.weights_batch_method == 'all':
            for embedding, shape in zip(positional_embeddings, predicted_weights_shapes):
                layer_reconstructed_weights = HANDPredictorBase._predict_weights(predictor, embedding).reshape(shape)
            ...
            reconstructed_weights = HANDPredictorBase._predict_all_by_layers(predictor,
            ...
            reconstructed_weights = HANDPredictorBase._predict_all_by_random_batches(predictor,
                                                                                     positional_embeddings,
                                                                                     original_weights,
                                                                                     predicted_weights_shapes)
        else:
    ...
    @staticmethod
    def _predict_all_by_random_batches(predictor: 'HANDPredictorBase', positional_embeddings: List[torch.Tensor],
        ...
        predicted_with_grads = HANDPredictorBase._predict_weights(predictor, stacked_embeddings[indices_with_grads])
        ...
        if indices_without_grads.shape[0] > 0:
            with torch.no_grad():
                predicted_without_grads = HANDPredictorBase._predict_weights(predictor,
    ...
    @staticmethod
    def _predict_all_by_layers(predictor: 'HANDPredictorBase', positional_embeddings: List[torch.Tensor],
    ...
    @staticmethod
    def _predict_weights(predictor: 'HANDPredictorBase', layer_positional_embeddings):
