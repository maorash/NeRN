...
def l2_layer_smoothness(layer_weights: torch.Tensor) -> torch.Tensor:
    ...
    if h > 1:
        normalized_layer_weights = resized_layer_weights / torch.norm(resized_layer_weights, dim=2,
                                                                      keepdim=True)
    else:
        normalized_layer_weights = resized_layer_weights / torch.norm(resized_layer_weights,
                                                                      keepdim=True)
...
def cosine_layer_smoothness(layer_weights: torch.Tensor) -> torch.Tensor:
    cout, cin, h, w = layer_weights.shape
    resized_layer_weights = layer_weights.reshape([cout, cin, -1])
    if h == 1:
        return -l2_layer_smoothness(layer_weights)
...
class CosineSmoothness(RegularizationBase):
    def forward(self, model: OriginalModel) -> torch.Tensor:
        ...
        for layer_weights in learnable_weights:
            total_cosine += cosine_layer_smoothness(layer_weights)
        return total_cosine
...
class L2Smoothness(RegularizationBase):
    def forward(self, model: OriginalModel):
        ...
        for layer_weights in learnable_weights:
            total_l2 += l2_layer_smoothness(layer_weights)
        return -total_l2
