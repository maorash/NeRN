...
class ResNet18(OriginalModel):
    def __init__(self, num_classes: int = 10):
        ...
        self.model.fc.weight = torch.nn.Linear(self.model.fc.in_features, self.num_classes)
    ...
    def layer_forward(self, layer: torch.nn.Sequential, x: torch.Tensor, extract_feature_maps: bool = False) -> (
    ...
    def forward(self, x, extract_feature_maps=False):
        ...
        x, layer1_activations = self.layer_forward(self.layer1, x, extract_feature_maps)
        x, layer2_activations = self.layer_forward(self.layer2, x, extract_feature_maps)
        x, layer3_activations = self.layer_forward(self.layer3, x, extract_feature_maps)
        x, layer4_activations = self.layer_forward(self.layer4, x, extract_feature_maps)
